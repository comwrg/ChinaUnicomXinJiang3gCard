    ...
    public class XinJiang3gCardHttp
    {
        ...
        public bool Order(ExportInfo ei)
        {
            ...
            RequestItem item = new RequestItem()
            {
                ...
                Method = Method.POST,
                PostData = $"paramStr={paramStr}&inventoryType={inventoryType}&uploadTokenUuid=&_m_token={mToken}&{Uri.EscapeDataString(hName)}={Uri.EscapeDataString(hValue)}"
            };
        ...
        {
            string articleCity = Regex.Match(XinJiang3gCardHttpRes.Attribution, $"(\\d{{3}}){attribution}").Groups[1].Value;
            RequestItem item = new RequestItem
            {
        ...
        public void PromptlyOrder(string goodsId, string attribution,out string mToken, out string hName, out string hValue, out decimal price)
        {
           ...
           // Console.WriteLine(res.String);
            ...
            price = Convert.ToDecimal(Regex.Match(res.String, "billingResult\" class=\"moneyQ\">(.*?)</").Groups[1].Value);
        }

        public void GetCityCode(string cityName, out string cityCode, out string essCityCode)
        {
...
//            districtCode = m.Groups[2].Value;
        ...
        public void GetDistrictCode(string cityCode, string districtName, out string districtCode)
        {
            var jss = JObject.Parse(XinJiang3gCardHttpRes.Code);
        ...
        public string GetAddrCode(string essCityCode, string districtCode, string addr)
        {
            RequestItem item = new RequestItem
            {
